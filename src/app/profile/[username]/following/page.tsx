...
export default async function FollowingPage({ params }: Props) {
  ...
  // Following: users that target follows
  const { data: followRows } = await (supabase
    .from('follows')
    .select('following:users!following_id ( id, username, display_name, bio, tags, avatar_url )')
    .eq('follower_id', target.id) as unknown as Promise<{
    data: { following: UserResult }[] | null
    error: unknown
  }>)

  const following: UserResult[] = (followRows ?? []).map((r) => r.following)
  ...
  const { data: viewerFollows } = await (supabase
    ...
    .in('following_id', following.map((f) => f.id)) as unknown as Promise<{
      ...
      <main className="max-w-2xl mx-auto px-6 py-8">
        ...
        {following.length === 0 ? (
          <div className="border border-border px-6 py-12 text-center">
            <p className="font-mono text-sm text-ink-faint">Not following anyone yet.</p>
          </div>
        ) : (
